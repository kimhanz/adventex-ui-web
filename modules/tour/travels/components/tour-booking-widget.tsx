...
function TourBookingWidget({
  ...
  function handleAdultChange(increment: number) {
  ...
  function handleBooking() {
    ...
    const tourCode = pathname.split("/").pop()
    const path = `/tours/travels/${tourCode}/booking/step-2?departureId=${selectedDeparture.id}`
    router.push(path)
  }
        ...
        {departureDates?.length === 0 ? (
          <>
            <h3 className="text-xl font-bold">
              แพ็คเกจทัวร์อยู่ระหว่างการเปิดตัว
            </h3>

            <div>
              <p>จะแจ้งล่วงหน้า 1 เดือนก่อนเดินทาง</p>
              <p>หากสนใจ ติดตามข่าวสารได้ทุกช่องทาง</p>
            </div>
          </>
        ) : (
          <>
            ...
            <Select
              ...
              onValueChange={(value) =>
                setSelectedDateIndex(parseInt(value, 10))
              }
